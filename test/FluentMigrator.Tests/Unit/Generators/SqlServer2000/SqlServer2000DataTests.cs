...
namespace FluentMigrator.Tests.Unit.Generators.SqlServer2000
{
    ...
    public class SqlServer2000DataTests : BaseDataTests
    {
        ...
        [Test]
        public virtual void CanUpsertDataWithSingleMatchColumnAndCustomSchema()
        {
            var expression = GeneratorTestHelper.GetUpsertDataExpression();
            expression.SchemaName = "TestSchema";

            var result = Generator.Generate(expression);
            result.ShouldContain("IF EXISTS (SELECT 1 FROM [TestSchema].[TestTable1] WHERE [Name] = N'Just''in')");
            result.ShouldContain("UPDATE [TestSchema].[TestTable1]");
            result.ShouldContain("INSERT INTO [TestSchema].[TestTable1]");
            result.ShouldContain("([Name], [Website])");
            result.ShouldContain("VALUES (N'Just''in', N'github.com')");
        }

        [Test]
        public virtual void CanUpsertDataWithSingleMatchColumnAndDefaultSchema()
        {
            var expression = GeneratorTestHelper.GetUpsertDataExpression();
        ...
        [Test]
        public virtual void CanUpsertDataWithMultipleMatchColumnsAndCustomSchema()
        {
            var expression = GeneratorTestHelper.GetUpsertDataExpressionWithMultipleMatchColumns();
            expression.SchemaName = "TestSchema";

            var result = Generator.Generate(expression);
            result.ShouldContain("IF EXISTS (SELECT 1 FROM [TestSchema].[TestTable1] WHERE [Name] = N'Just''in' AND [Category] = N'Developer')");
            result.ShouldContain("UPDATE [TestSchema].[TestTable1]");
            result.ShouldContain("INSERT INTO [TestSchema].[TestTable1]");
        }

        [Test]
        public virtual void CanUpsertDataWithMultipleMatchColumnsAndDefaultSchema()
        {
            var expression = GeneratorTestHelper.GetUpsertDataExpressionWithMultipleMatchColumns();

            var result = Generator.Generate(expression);
            result.ShouldContain("IF EXISTS (SELECT 1 FROM [TestTable1] WHERE [Name] = N'Just''in' AND [Category] = N'Developer')");
            result.ShouldContain("UPDATE [TestTable1]");
            result.ShouldContain("INSERT INTO [TestTable1]");
        }
        ...
        public virtual void CanUpsertDataWithSpecificUpdateColumns()
        {
            ...
            var result = Generator.Generate(expression);
            result.ShouldContain("IF EXISTS (SELECT 1 FROM [TestTable1] WHERE [Name] = N'Just''in')");
            result.ShouldContain("SET [Website] = N'github.com'");
            result.ShouldNotContain("SET [Email] = N'test@example.com'"); // Should not update non-specified columns
            result.ShouldContain("VALUES (N'Just''in', N'github.com', N'test@example.com')"); // But insert should include all columns
        }

        [Test]
        public virtual void CanUpsertMultipleRows()
        {
            var expression = GeneratorTestHelper.GetUpsertDataExpressionWithMultipleRows();

            var result = Generator.Generate(expression);
            result.ShouldContain("IF EXISTS (SELECT 1 FROM [TestTable1] WHERE [Name] = N'Just''in')");
            result.ShouldContain("IF EXISTS (SELECT 1 FROM [TestTable1] WHERE [Name] = N'Jane')");
            result.ShouldContain("VALUES (N'Just''in', N'github.com')");
